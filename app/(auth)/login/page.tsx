...
function LoginForm() {
  ...
  const onSubmit = async (values: LoginValues) => {
    ...
    try {
      ...
      // redirect is handled by server action signIn
    } catch {
  ...
  return (
    <div className="flex flex-col items-center py-12">
      ...
      <div className="w-full text-center mb-10">
        ...
        <p className="font-sans text-[15px] text-foreground/60">Tu historia te espera.</p>
      </div>

      {successMessage && (
        <div className="w-full mb-6 rounded-xl bg-green-50 p-3 text-center text-[13px] text-green-700 border border-green-100">
          {successMessage}
        </div>
      )}
      ...
      <p className="mt-8 text-[14px] text-foreground/60">
        ...
        <Link href="/register" className="font-semibold text-foreground underline decoration-border underline-offset-4 transition-colors hover:decoration-foreground">
          Regístrate
        </Link>
      </p>
    </div>
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="flex h-screen items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-foreground/20" /></div>}>
      <LoginForm />
    </Suspense>
  );
}
