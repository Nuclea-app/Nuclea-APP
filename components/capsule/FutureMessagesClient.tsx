...
interface FutureMessagesClientProps {
  ...
  capsuleName: string;
  capsuleId: string;
  /** Override base path for message links. Defaults to /dashboard/capsula/{capsuleId}/mensajes-futuros */
      ...
      {list.length === 0 ? (
        ...
        <div className="space-y-3">
          {list.map((m) => (
            <Link
              key={m.id}
              href={`${messageBasePath ?? `/dashboard/capsula/${capsuleId}/mensajes-futuros`}/${m.id}`}
