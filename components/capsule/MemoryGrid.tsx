  ...
  return (
    <div className="w-full">
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar -mx-2 px-2">
        {memories.length === 0 ? (
          <div className="flex h-[100px] w-full items-center justify-center rounded-2xl border border-border shadow-sm">
          ...
          memories.map((memory) => (
            <div
              key={memory.id}
              className="relative shrink-0 w-[120px] h-[120px] border border-border rounded-2xl overflow-hidden shadow-2xl"
            >
              <FavoriteButton
                memoryId={memory.id}
              ...
              {memory.type === "VIDEO" &&
                (memory.fileUrl ? (
                  ...
                  <div className="flex items-center justify-center h-full">
                    <Video className="h-6 w-6 opacity-10" />
                  </div>
                ))}
              ...
              {memory.type === "AUDIO" &&
                (memory.fileUrl ? (
                  ...
                  <div className="flex items-center justify-center h-full">
                    <Mic className="h-6 w-6 opacity-10" />
                  </div>
                ))}
              ...
              {memory.type === "NOTE" && (
                <div className="flex flex-col h-full p-3 gap-2 bg-surface/50">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[8px] font-medium tracking-widest uppercase text-foreground/30">
                      NOTA ✦
                    </span>
                  </div>
